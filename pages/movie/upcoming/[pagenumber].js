...
import { getUpcomingMoviesByPage } from '../../../utils/data/themoviedb';
...
export default function UpcomingMoviePage() {
  ...
  if (pageNumber <= 0) {
    ...
    pageNumber = 1;
    router.push('/movie/upcoming/1');
  }

  useEffect(() => {
    getUpcomingMoviesByPage(pageNumber).then((data) => {
  ...
  const handleNextPage = () => {
    ...
    router.push(`/movie/upcoming/${nextPage}`);
  };

  const handlePrevPage = () => {
    if (pageNumber > 1) {
      const prevPage = pageNumber - 1;
      router.push(`/movie/upcoming/${prevPage}`);
    }
  };
  ...
  return (
    <Container className="text-center pushdown-top">
      ...
      <span style={{ color: 'red' }}>Please Note:</span> The accuracy of TMDB upcoming release information may not always align with release date order.
      <Row>
        {movies.map((movie) => (
